...
export const MessageInput = () => {
  ...
  const send = async () => {
    if (isSubmitDisabled || !conversation) {
      log.error("Failed sending message: no conversation or invalid input");
      return;
    }
  ...
  const onFocus = () => conversation?.setAllMessagesRead();
  ...
  useEffect(() => {
    if (attachmentsBoxRef.current && attachedFiles.length > oldAttachmentsLength.current) {
  ...
  return (
    <Box as="form" className={classes.form} onSubmit={(e) => { e.preventDefault(); send(); }}>
      ...
      {attachedFiles.length > 0 && (
        <Box data-test="message-attachments" className={classes.filePreviewContainer} ref={attachmentsBoxRef}>
          {attachedFiles.map((file, index) => (
            <FilePreview key={index} focusable={true} file={file} isBubble={false} disabled={isSending} />
          ))}
        </Box>
      )}
    </Box>
